...
def convert_all(values, currencies, year):
    """Conversion from multiple currencies to USD with corrected inflation of the year
    ...
    output:
        values, currencies with corrected values and symbol
    """
    converter = CurrencyConverter()

    values, currencies = convert_usd_usd(currencies, values, year)
    for currency in CURRENCY_META:
        values, currencies = convert_other_usd(
            currency, CURRENCY_META[currency], currencies, values, year, converter
        )
